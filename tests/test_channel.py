...
def test_channels_create_and_list_all(clear):
	...
	with pytest.raises(ValueError):
		channel_3 = channels_create(
			token, "channelfdahfldkajdfhlakdjfhalkdjfhlajdf", True)
	...
	with pytest.raises(AccessError):
		channel_3 = channels_create(
			token2, "channelfdahfldkajdfhlakdjfhalkdjfhlajdf", True)
	with pytest.raises(ValueError):
		channel_3 = channels_create(
			token, "channelf2adlfkj;adlkfj;dlsakj;akdsf", True)
...
def test_channel_invite(clear):
	...
	login3 = auth_register(
		"jason@gmail.com", "dafadkh;lktlk444", "Jason", "Xing")
	userID3 = login3["u_id"]
	token3 = login3["token"]
